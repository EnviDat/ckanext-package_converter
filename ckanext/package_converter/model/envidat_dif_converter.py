...
class GcmdDifConverter(BaseConverter):
    ...
    def convert(self, record):
        if self.can_convert(record):
            ...
            # (custom_replace=[('xs:include schemaLocation="U', 'xs:include schemaLocation="'+ site_url
            # +'/package_converter_xsd/U')])) + ' ****')
    ...
    def _dif_convert_dataset(self, dataset_dict):
        ...
        dif_metadata_dict['@xsi:schemaLocation'] = '{namespace} {schema}'.format(
        ...
        dif_metadata_dict['Dataset_Citation']['Data_Presentation_Form'] = ','.join(
            self._get_resource_formats(dataset_dict))
        ...
        dif_metadata_dict[
            'Use_Constraints'] = 'Usage constraintes defined by the license "' + license.strip() + '", see ' + license_url
